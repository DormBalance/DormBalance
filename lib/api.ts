...
import { CreateExpenseRequest, ExpenseFilters, GetExpenseResponse, CreateExpenseResponse, HouseholdMember, GetRecurringExpenseResponse, RunDueResponse} from '@/types';
...
export async function getExpenseCategories(householdId: string): Promise<Result<GetExpenseCategoryResponse[]>> { //this function was partially generated by github copilot but then modified based on existing get Expense Category function.
  try{
    let url = `/api/expense_category?household_id=${encodeURIComponent(householdId)}`;
    let response = await fetch(url);
...
export async function getHouseholdMembers(userID: string): Promise<Result<HouseholdMember[]>> {
  try{
    let url = `/api/household_members?userId=${encodeURIComponent(userID)}`;
    let response = await fetch(url);
